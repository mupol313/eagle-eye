...
def load_pair_history(pair: str,timeframe: str,datadir: Path, *,timerange: Optional[TimeRange] = None,fill_up_missing: bool = True,
                        drop_incomplete: bool = False,startup_candles: int = 0,data_format: Optional[str] = None,data_handler: Optional[IDataHandler] = None,candle_type: CandleType = CandleType.SPOT) -> DataFrame:
...
def download_data_main(config: Config) -> None:
    ...
    markets = exchange.get_markets(tradable_only=True, active_only=not config.get('include_inactive'))
    available_pairs = [p for p in markets.keys()]  # pairs that is available in exchange

    expanded_pairs = dynamic_expand_pairlist(config, available_pairs) # pairs that is in the config  pair-whitelist

    if 'timeframes' not in config:
        config['timeframes'] = DL_DATA_TIMEFRAMES

    if not config['exchange'].get('skip_pair_validation', False):
        exchange.validate_pairs(expanded_pairs)    # checked that the config whie list pairs validate in the exchange markets
    logger.info(f"About to download pairs: {expanded_pairs}, "     # if any of pairs is not in the tradeble pairs of exchange raised
    ...
    for timeframe in config['timeframes']:
        exchange.validate_timeframes(timeframe)   # check the config time frame with exchange timeframes if any of the ti,e frames are not in the exchange raised
    try:
